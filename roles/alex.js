  ...
  const def = {
    ...
    cards: [
      ...
      // ── EARLY: MARKET & IDEA ────────────────────────────────────────────────
      {
        ...
        options: [
          { label: 'Pivot — investigate upstream', key: 'pivot',
            execute(s, char) { char.flags.interview_shock_resolved = true; s.signal = clamp(s.signal + 15, 0, 100); s.product = clamp(s.product - 5, 0, 100); s.market_fit = clamp(s.market_fit + 14, 0, 100); return "Pivoted focus. Lost a week of progress but gained real signal. Three more interviews confirmed it."; } },
      ...
      },
      {
        ...
        options: [
          { label: 'Pivot — rethink the approach', key: 'pivot',
            execute(s, char) { char.flags.pivot1 = true; s.market_fit = clamp(s.market_fit + 25, 0, 100); s.product = clamp(s.product - 5, 0, 100); s.signal = clamp(s.signal + 8, 0, 100); return "Rethought the approach. Redirected some progress but found the real problem. Signal improved immediately."; } },
          { label: 'Stay the course', key: 'stay',
      ...
      },
      {
        ...
        body: "second round done. consistent feedback — they want depth on one thing, not breadth. our current scope is too wide and they're not seeing the core value.",
        urgency: 3, weeks: 1,
        ...
        options: [
          { label: 'Narrow scope — go deep', key: 'pivot',
            execute(s, char) { char.flags.pivot2 = true; s.market_fit = clamp(s.market_fit + 20, 0, 100); s.product = clamp(s.product - 3, 0, 100); s.signal = clamp(s.signal + 10, 0, 100); return "Narrowed scope significantly. Less ambitious but far more right. Three users asked for exactly this."; } },
