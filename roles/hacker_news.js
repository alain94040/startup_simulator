  ...
  const def = {
    ...
    cards: [
      ...
      // at the same time.
      {
        ...
        urgency: 1, weeks: 1,
        available: (s) => !s.launched && s.week >= 6 && s.week >= (s.community_hn_last || 0) + 4,
        options: [
          { label: 'Drop a comment', key: 'engage',
            execute(s) { s.community_hn_last = s.week; s.signal = clamp(s.signal + 6, 0, 100); s.network.peers += 3; s.market_fit = clamp(s.market_fit + 3, 0, 100); return "Commented with a genuine take. 4 people DM'd asking when you're launching."; } },
...

      {
        ...
        urgency: 1, weeks: 1,
        available: (s) => !s.launched && s.week >= 10 && s.week >= (s.community_slack_last || 0) + 4,
        options: [
          { label: 'Share what you\'ve learned', key: 'engage',
            execute(s) { s.community_slack_last = s.week; s.signal = clamp(s.signal + 5, 0, 100); s.network.peers += 4; s.market_fit = clamp(s.market_fit + 3, 0, 100); return "Posted an honest update. 5 people followed up with their own experiences. Real signal."; } },
          { label: 'Skip it', key: 'skip',
