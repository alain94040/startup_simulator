(function () {
  ...
  const def = {
    ...
    unlockCondition: (s) => s.customers >= 3,
    cards: [
      {
        ...
        urgency: 3, weeks: 1,
        available: (s) => s.launched && s.customers >= 3 && s.week >= (s.bug_reports_last || 0) + 5,
        options: [
        ...
        dropFx(s) { s.bug_reports_last = s.week; s.customers = clamp(s.customers - 10, 0, 9999); s.signal = clamp(s.signal - 10, 0, 100); },
      },
      {
        ...
        urgency: 2, weeks: 2,
        available: (s) => s.launched && s.customers >= 5 && s.week >= (s.feature_cluster_last || 0) + 5,
        options: [
