      ...
      // ── EARLY: MARKET & IDEA ─────────────────
      {
        ...
        options: [
          { label: 'Pivot — investigate upstream', key: 'pivot',
            execute(s, char) { char.flags.interview_shock_resolved = true; s.signal = clamp(s.signal + 15, 0, 100); s.product = clamp(s.product - 5, 0, 100); return "Pivoted focus. Lost a week of progress but gained real signal. Three more interviews confirmed it."; } },
          { label: 'Stay the course', key: 'stay',
            execute(s, char) { char.flags.interview_shock_resolved = true; s.signal = clamp(s.signal + 5, 0, 100); return "Filed it away. Not ready to pivot on one data point. Logged it for later."; } },
        ],
        dropDelay: 2, dropFrom: 'Alex',
      ...
      },
      {
        ...
        options: [
          { label: 'Rewrite the outreach', key: 'rewrite',
            execute(s, char) { char.flags.cold_silence_resolved = true; s.signal = clamp(s.signal + 10, 0, 100); return "Rewrote the cold email. New version leads with the pain, not the product. First reply came in 4 hours."; } },
        ],
        dropDelay: 2, dropFrom: 'Alex',
      ...
      },
      {
        ...
        options: [
          { label: 'Test the new framing', key: 'test',
            execute(s, char) { char.flags.reframe_resolved = true; s.signal = clamp(s.signal + 12, 0, 100); s.network.peers += 3; return "Ran the new framing by 3 more people. All 3 immediately got it. Updated the positioning."; } },
        ],
        dropDelay: 0, dropMsg: null, dropFx: null,
      },
      {
    ...
    cards: [
      {
        ...
        options: [
          { label: 'Do a competitive deep-dive', key: 'research',
            execute(s, char) { char.flags.competitor_resolved = true; s.signal = clamp(s.signal + 8, 0, 100); s.network.advisors++; return "Did a full competitive analysis. None of them solve it for your niche. That's your wedge. Priya is now a real advisor."; } },
        ],
...
const SITUATIONS = [
  ...
  },
  {
    ...
    dropDelay: 0, dropMsg: null, dropFx(s) { s.hn_thread_done = true; s.signal = clamp(s.signal - 3, 0, 100); },
  },
  {
  ...
  },
  {
    ...
    urgency: 3, weeks: 1,
    available: (s) => s.product > 20 && !s.competitor_launch_done,
    options: [
      { label: 'Write a comparison piece', key: 'compare',
        execute(s) { s.competitor_launch_done = true; s.signal = clamp(s.signal + 6, 0, 100); return "Published a direct comparison. Your niche is clearer. A few users switched from Rivalio to you."; } },
    ],
    ...
    dropFx(s) { s.competitor_launch_done = true; s.signal = clamp(s.signal - 12, 0, 100); },
  },
  {
    ...
    urgency: 1, weeks: 1,
    available: (s) => s.incorporated && s.hn_thread_done && s.week >= 4 && !s.consultant_brand_done,
    options: [
      { label: 'Book the session — $1,500', key: 'hire',
        execute(s) { s.consultant_brand_done = true; s.cash = clamp(s.cash - 1500, 0, 9999999); s.product = clamp(s.product - 3, 0, 100); return "45 minutes of sticky notes and a 'narrative architecture' framework. Brett's main insight: 'lean into your why.' You already knew this. He emailed his invoice immediately."; } },
      { label: 'Pass on this one', key: 'pass',
        execute(s) { s.consultant_brand_done = true; return "Declined politely. Brett replied: 'no worries — here's my newsletter.' Subscribed automatically."; } },
    ],
    dropDelay: 1, dropFrom: 'Brett',
    dropMsg: "just wanted to follow up — offer stands. a lot of founders wait too long on this.",
    dropFx(s) { s.consultant_brand_done = true; },
  },
  {
    ...
    urgency: 1, weeks: 1,
    available: (s) => s.launched && s.customers >= 3 && !s.consultant_growth_done,
    options: [
      { label: 'Hire Kevin for a week — $2,000', key: 'hire',
        execute(s) { s.consultant_growth_done = true; s.cash = clamp(s.cash - 2000, 0, 9999999); return "Kevin delivered a 58-slide deck titled 'Growth Architecture 2.0'. His top recommendation: post more on LinkedIn. He invoiced before the final call."; } },
      { label: 'Not the right time', key: 'pass',
        execute(s) { s.consultant_growth_done = true; return "Replied it's too early for paid growth work. Kevin: 'it's never too early for growth.' Unsubscribed."; } },
    ],
    ...
    dropFx(s) { s.consultant_growth_done = true; },
  },
  {
  ...
  },
  {
    ...
    urgency: 2, weeks: 1,
    available: (s) => !s.launched && s.signal > 45 && s.network.peers >= 14 && !s.waitlist_cold_done,
    options: [
      { label: 'Reach out now', key: 'reach',
        execute(s) { s.waitlist_cold_done = true; s.customers += 8; s.signal = clamp(s.signal + 8, 0, 100); return "Reached out to waitlist. 8 became active beta testers."; } },
    ],
    dropDelay: 2, dropFrom: 'Waitlist',
    dropMsg: "signed up a few weeks ago. assumed the product was dead. unsubscribed.",
    dropFx(s) { s.waitlist_cold_done = true; s.signal = clamp(s.signal - 10, 0, 100); },
  },
  {
