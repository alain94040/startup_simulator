...
- **`scoring.js`** — the endgame report card. Pure logic, no DOM; dual export (`module.exports` / `window.Scoring`). `scoreGame(engine)` grades a run on 10 educational dimensions (equity conversation, co-founder care, firing Jordan, F&F money, reading investors, lean scope, build-vs-buy, customer discovery, the pivot, runway discipline) from `engine.s` flags, `engine.log` (acted cardId+option, `ignored`, `surfaced` entries), and `engine.ledger`; each category carries a 📚 reference to the canonical startup writing it teaches (PG essays, Wasserman, Spolsky, Blank, Andreessen, Ries). Categories the player never got to face return `score:null` ("never faced"). Safe to call in any game state — `tests/test_narrative.js` smoke-checks it at every game end.
