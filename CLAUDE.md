...
The content set: **~172 nodes, 18 cast, 6 arcs (5 of them scenes)** across the `story/` files: `opening`, `equity`, `dev_plan`, `team`, `dev_directions`, `demo_night`, `users`, `launch_day`, `slide`, `pivot_day`, `community`, `fundraising`, `growth`, `jordan_arc`, `firing`, `discovery`, `press`, `ambient`.
...
- `choices[]` — each `{ key, label, reply?, replyTo?, payee?, if?, journal?, effects?, fx? }`. `reply` lands as an outgoing bubble in the answering character's thread; `replyTo: "<charId>"` sends it to a different thread instead — which is what a decision made on the founder's own "Your move" card needs, since the founder thread renders no bubbles at all. `payee` overrides the Bank ledger's label for that choice's cash effect — the default is the card's sender (`node.from || char.def.name`), which is wrong when the sender only raises a cost that's actually paid to someone else (Alex flags the cap table; the $2k goes to `payee: "Lawyer"`, not to Alex). A per-option `if(s,e,char)` gates it (this is how the research-gated "C-options" appear only when the player has learned enough).
...
`{ cash, signal, marketFit, waitlist, users, customers, saas, flags, char{ morale, trust, effort, focus, flags }, say, note, schedule, surface, scene }`. Cash deltas auto-log to `engine.ledger` (the Bank statement). `effects.schedule` queues a delayed consequence (`{ in, char?, say?, effects?, fx?, unless? }`), fired in `nextWeek()`. `effects.surface = "<nodeId>"` pulls a named node into the **current** week — the opt-in exception to the boundary rule, for a beat an answer directly causes (filing the incorporation is what makes the split a live question, so Jordan's opener lands the same week). `effects.scene = arcId` enters a scene; `scene: null` exits it. `effects.note` renders a rare, explicitly-authored aside inline in the node's own thread — styled unlike a chat bubble on purpose (see *Coding conventions*), reserved for a detail (usually a verbatim quote) the thread has no other way to carry.
...
**The horizon:** every run ends entering `s.deadline_week` (25) — there is no application to write, no wait for a letter: `world.js`'s weekly tick grades the run on the spot the moment the deadline arrives, and the report card prints immediately. Admission needs **both** the grade (`engine.gradeScore()` ≥ 80, a B+) **and** the traction bar (`launched && pivot_shipped && customers >= 1` — wise answers without a shipped company don't get funded). **Win:** YC acceptance (`s.ycAccepted`, which also pays out $500k and bumps signal). **Lose:** the grade or traction bar not cleared at the deadline (`s.deadline_passed`), cash hitting $0, or **losing Alex** (`s.cofounder_left`) — neglecting the technical co-founder until he takes the other offer ends the run on the spot, like bankruptcy. One founder cannot build, launch, pivot and sell inside the horizon, so there is no solo mode; the report card prints immediately and grades accordingly. **Starting cash** $10,000; **burn** `burnPerWeek = 500 + s.extra_burn` (recurring SaaS costs from build-vs-buy choices).
...
| 4 · Rebuild as v2 | Deal with the co-founder the pivot made redundant, then ship (the Jordan arc gates the relaunch) | the firing scene → v2 relaunch (on a chosen stage) |
| 5 · Make the case | Prove it before week 25: revenue, a channel, a clean cap table | The deadline → the verdict, graded on the spot |
...
- **The Jordan arc gates the relaunch, and that is the ordering.** The pivot
  obsoletes Jordan's work (`applyActivitiesPivot` marks `matching_algo`/`ios_ui`
  obsolete) and hands her `plans_ui` — the one screen v2 is built around.
  `world.js` will **not** complete `plans_ui` while she is drifting and still on
  the team (`s.jordan_blocking_ui`), so Alex's half lands, hers visibly does not,
  and he escalates until the player has the conversation. Firing her hands the
  screen to Alex and unblocks `pivot_relaunch`. A founder who never has the
  conversation is not soft-locked — `pivot_relaunch_rough` ships v2 on Alex's
  stand-in screen, later, converting at half (`world.js` `roughShipPenalty`) and
  grading as a half-shipped pivot. **When touching this arc, keep every gate
  closed on `s.jordan_quit` as well as `s.jordan_resolved`**: a quit leaves
  `jordan_resolved` false, and a re-entered firing scene with an inactive Jordan
  can surface no beat, never exits, and holds every other character's slot.
- **Failure is evidence.** Trying the wrong fix is never purely punished: the traffic push, the feature spree, and the win-back blast each bank a flag that becomes a playable chip later. This is the game's core currency — **facts banked in one chapter become the chips of a later exam**. There are two sinks: pivot day (chapter 3's climax, arguing Alex off his whiteboard, a scene where the evidence chips are argued live) and the report card itself (chapter 5's — no scene, no bluff to write: `scoring.js` reads whatever the run actually banked — Maya's quote, the cohort, the failed fixes, the channel tests — directly off the facts ledger the moment the deadline grades the run). New evidence sources should plug into one of these two sinks, or they're trivia.
- **Chapters end on decisions, not events.** Ch 2 ends choosing the splash before the switch; Ch 4 ends choosing the relaunch stage (Sarah's event / the press second-bite / a quiet update); Ch 5 ends on the deadline itself, graded automatically off whatever the run banked. The mechanical transition (launch, ship) should always arrive *through* a player call, so the boundary is something they did, not something that happened to them.
...
An arc with `scene: { cast: [...] }` is a **war-room** (there are 5: `equity`, `demo`, `launch`, `pivot`, `firing` — the last is a one-on-one, and the only one whose cast is a single character plus the founder). Entering it (`effects.scene = arcId`) makes only its cast surface, re-polls after every answer so the talk flows in one sitting, and holds the rest of the world. **The action economy of a room: entering one is answering a message, so it costs one of the week's two moves — and that one move buys the whole sitting.** Every beat answered once you are inside is free (`inSceneBefore` in `act()`); the answer that opens the room is not, whether the content authored it as the arc's own first beat (`equity_open`, `demo_ready`) or as a standalone card (`pivot_summit_call`, `good_enough_launch`, `jordan_confrontation`). Each beat is authored on its own character's thread — scenes are **not** one merged transcript.
