// ─────────────────────────────────────────────────────────────────────────────
// chat_engine.js — thin chat-simulation coordinator.
//
...
// character owns its own curation (`def.slice`), priorities (`card.priority`),
...
// roles/*.js. No DOM here. Dual export: Node (module.exports) + browser (window.ChatEngine).
// ─────────────────────────────────────────────────────────────────────────────
...
(function () {
  ...
  class ChatEngine {
    ...
    // otherwise the character's own numeric `priority` (default 0) decides.
    _rankVal(card) {
      return card.fallback ? -1 : (card.priority || 0);
    }
    // Does card `a` outrank `b` for the single slot? priority first, then urgency.
    _better(a, b) {
      const ra = this._rankVal(a), rb = this._rankVal(b);
      if (ra !== rb) return ra > rb;
      return (this._urgency(a) || 0) > (this._urgency(b) || 0);
    }
    // Select-by-priority API: the best currently-available card from `cards`, or null.
    pick(cards, char) {
      const avail = cards.filter(c => c.available(this.s, char, this));
      if (!avail.length) return null;
      avail.sort((a, b) =>
        this._rankVal(b) - this._rankVal(a) ||
        (this._urgency(b) || 0) - (this._urgency(a) || 0));
      return avail[0];
    }
