"use strict";
// ─────────────────────────────────────────────────────────────────────────────
// v2/tests/phase_map.js — simulation-driven pacing / phase-timeline extractor
// for the v2 engine.
//
// Plays many games and records when each arc beat happens, the gaps between
// beats, decision density, and where the game goes quiet. The v2 facts ledger
...
//
// New vs the old tool: a "left on read" section measuring how much of the
// story resolves by @ignored timeout — the action-starvation signal.
//
// Usage:
//   node v2/tests/phase_map.js                 # default (decent+pivot)
//   node v2/tests/phase_map.js 500             # games per driver (default 300)
//   node v2/tests/phase_map.js --driver pivot
//   node v2/tests/phase_map.js --all           # pool all games (no winner cohort)
//   node v2/tests/phase_map.js --subsidy 500   # test-only weekly stipend
//
...
function report(records, opts) {
  ...
  console.log(`\nPhase pacing (v2) — ${all.length} games (${opts.drivers.join(", ")})` +
  ...
  // left on read — the action-starvation signal (new in v2)
