...
**Historical design log.** This is the record of the rewrite that produced the
engine now shipping at the repo root, written pass by pass while it happened.
The card-dealer engine it argues against (`roles/*.js`, a single `urgency` axis)
no longer exists in the repo — it was deleted once the rewrite shipped, and the
new engine moved up from its own subdirectory to the root. Read this for *why* the current
design is shaped the way it is; read `CLAUDE.md` for what is actually there.
...
### Pass 4b — game.html (the playable UI)
...
- **UI feel**: judge the messenger direction in real play; the user remains
  "not fully convinced."
- **Strategy-system redesign** (the current `sim_strategies.js` is a placeholder).

## How to run
- Play: `open game.html` (or `?seed=42` for a reproducible run;
  `play.html` for the plain debug harness).
- Regressions: `node tests/test_slice.js`, `node tests/test_narrative.js`,
  `node tests/test_scenes.js`.
- Balance/pacing: `node tests/phase_map.js [--subsidy 500]`,
  `node tests/sim_strategies.js`.
