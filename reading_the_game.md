...
Every tool in `tests/` reports **statistics about runs** — pacing quantiles,
...
## What's built: `tests/transcript.js`

A headless run, replayed in order as something you read.

```
node tests/transcript.js                      # seed 42, the decent founder
node tests/transcript.js --compact            # skim the whole run, ~430 lines
node tests/transcript.js --driver no_pivot    # a named archetype
node tests/transcript.js --char alex          # one relationship, in context
node tests/transcript.js --src                # every beat tagged story/file:line
node tests/transcript.js --sample --html book.html
node tests/transcript.js --audit
```
